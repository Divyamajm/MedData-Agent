"""
MedData AI - Safety & Guardrails Layer
...
# Known database columns in the Doctors table
DATABASE_KNOWN_FIELDS = {
    ...
    "is_available_today", "next_available_date", "id"
}
...
# Patterns asking for attributes that do NOT exist in our SQLite schema
UNKNOWN_ATTRIBUTE_PATTERNS = [
    ...
    (r"\b(insurance|in-network|blue cross|aetna|medicare|medicaid|cigna|unitedhealthcare)\b", "Insurance plan acceptance"),
    ...
    for pattern in MEDICAL_DIAGNOSIS_PATTERNS:
        if re.search(pattern, prompt_clean):
            return {
                ...
                "message": (
                    "⚠️ **Medical Safety Notice**: I am an AI doctor discovery and directory agent, "
...
def check_acute_emergency(prompt: str) -> Optional[Dict[str, Any]]:
    ...
    if any(term in prompt_clean for term in ["nearest", "closest", "how far", "cost", "fee", "doctors in emergency", "find doctor", "list"]):
        # Check if it's explicitly asking for doctor search despite mentioning emergency
        if re.search(r"\b(nearest|closest|doctor|specialist|fee|cost|directory|list|how far)\b", prompt_clean):
            return None
...
def check_unknown_attributes(prompt: str) -> Optional[Dict[str, Any]]:
    ...
    for pattern, description in UNKNOWN_ATTRIBUTE_PATTERNS:
        if re.search(pattern, prompt_clean):
    ...
    if missing_fields:
        ...
        return {
            ...
            "message": (
                ...
                "📊 **Available Verified Fields in Database:**\n"
                ...
                "* Distance (miles)\n"
                "* Consultation Fee ($)\n"
                "* Demo Availability (Today / Next Available Date)"
            )
        }
    return None
        ...
        if re.search(pattern, prompt_clean):
            return {
                ...
                "message": (
                    ...
                    "Please submit a valid healthcare directory or doctor discovery query."
                )
            }
    return None
...
def validate_sql_sandbox_query(query: str) -> Tuple[bool, str]:
    ...
    Rejects any mutation, DDL, or administrative commands.
    """
