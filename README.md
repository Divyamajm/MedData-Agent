...
### Grounded Healthcare & Housing Discovery Engine with Dual-Engine Intent Parsing, AST SQL Sandboxing & Bounded LLM Triage
...
## 🎯 Architecture Overview & The "Zero-Hallucination" Design Invariant

In high-stakes clinical and real estate discovery, allowing an unconstrained LLM to directly write or execute arbitrary SQL queries against a production database introduces severe hallucination and injection vulnerabilities.

**MedData AI** implements a **strict separation of concerns**:
1. **Input Boundary**: Natural language queries are parsed into structured JSON filter objects conforming to a strict **Pydantic v2 Schema**.
   - **Dual-Engine Triage**: Supports high-speed deterministic regex/dictionary classification (`<0.3ms` latency) or bounded LLM function calling (**Google Gemini / OpenAI**).
...
3. **Deterministic Query Compiler**: Converts validated Pydantic models into parameterized SQL queries with strict column allowlists (`ALLOWED_DOCTOR_COLUMNS`, `ALLOWED_SORT_METRICS`). **The LLM never touches, writes, or sees SQL.**
...
```
                              ...
                              │  • Acute Emergency Protocol (112/108)  │
...
## 📊 Scientific Evaluation Benchmark (265 Labeled Queries)

The repository includes an empirical, scientific evaluation suite (`python -m tests.eval_benchmark`) measuring precision, recall, accuracy, and latency across 265 multi-domain queries:

| Metric | Measured Score | Evaluation Focus |
|---|:---:|---|
| **Intent Classification Accuracy** | **88.8%** | Multi-intent disambiguation (search, directory, affordability, ranking). |
| **Entity Extraction Precision** | **98.8%** | Specialty, fee thresholds, city names, and distance radius matching. |
| **Clinical Safety Refusal Precision** | **87.1%** | Intercepting medical diagnosis, medication questions, and emergency symptoms. |
| **Ambiguity Interception Rate** | **91.7%** | Intercepting subjective queries (*"best doctor"*, *"top hospital"*) for clarification. |
| **SQL Execution & Grounding Rate** | **100.0%** | Zero SQL syntax errors, 100% database grounding with no fabricated rows. |
| **Mean Execution Latency** | **0.20 ms** | Deterministic AST compilation benchmark (p50: 0.12ms, p95: 0.57ms). |
...
#### Sample `curl` Request:
```bash
curl -X POST "http://localhost:8000/api/v1/triage/query" \
...
### 5. Execute Test Batteries:
```bash
# Run Core 32-Case Verification Suite & SQL Sandbox Tests:
python -m tests.test_suite

# Run 265-Query Scientific Evaluation Benchmark:
